...
class NeuralODE(nn.Module):
    ...
    def normalization_init(self, dataset: h5py.File):
        ...
        # states derivative
        self.NeuralODEFunc.normalization_states_der.initialize_normalization(reshape_array(dataset['train']['states_der'][:]))
    ...
    def get_progress_string(self, ret_vals_train, ret_vals_validation, ret_vals_test, pre_train):
        if pre_train is True:
            ...
            try:
                _str =  '[train/val/test] loss: {:.5f}/{:.5f}/{:.5f} | loss_states: {:.5f}/{:.5f}/{:.5f} | loss_outputs: {:.5f}/{:.5f}/{:.5f} | time_forward: {:.5f} | time_backward: {:.5f}'.format(
                    ret_vals_train['loss'], ret_vals_validation['loss'], ret_vals_test['loss'],
