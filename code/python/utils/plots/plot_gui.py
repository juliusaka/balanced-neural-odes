...
def plot_gui(dataset_path: Path, n_trajectory_rows: int = 3):
    ...
    class IdxMapper():
        def __init__(self, dataset: h5py.File) -> None:
            self.data_types = ['states', 'parameters', 'controls', 'outputs']
            self.context_keys = ['train', 'common_test']
            self.data_type_exist = {}
            for data_type_key in ['states', 'parameters', 'controls', 'outputs']:
                self.data_type_exist[data_type_key] = True if data_type_key in dataset['train'] else False
    ...
    fig, ax = plt.subplots(n_rows, 1, figsize=(10, 10))
    ...
    sample_slider_ax = plt.axes([pos.x0, pos.y0 + 0.22, pos.width, 0.03], facecolor=axcolor)
